    ...
    try:
        ...
        if result["status"] == "success":
            ...
            if request.process_immediately and FULL_PIPELINE_AVAILABLE:
                ...
                result: dict = process_recorded_audio(
                    result["file_path"], 
                    session_id, 
                    request.topic_hint
                )
...
def process_recorded_audio(audio_file_path: str, session_id: str, topic_hint: Optional[str] = None):
    ...
    try:
        ...
        script_data = script_gen.generate_script(transcript, topic_hint)
        
        # Step 3: Create narration
        narrator = Narrator()
        narration_path = narrator.create_narration(script_data['full_text'])
        
        # Step 4: Create animations
        animator = VideoAnimator()
        animation_path = animator.create_animation(script_data)
        
        # Step 5: Merge video and audio
        merger = VideoMerger()
        final_video_path = merger.merge_audio_video(animation_path, narration_path)
        
        # Step 6: Generate PDF
        pdf_gen = PDFGenerator()
        pdf_path = pdf_gen.create_pdf(script_data)
