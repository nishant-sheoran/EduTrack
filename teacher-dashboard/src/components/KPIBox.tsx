...
export default function KPIBox({ title, value, delta, icon, accentColor = "text-emerald-400" }: KPIBoxProps) {
  ...
  if (title === "Live Attendance" || title === "Live Engagement" || title === "Students in Frame") {
    ...
    return (
      <div className="bg-gray-800 rounded-xl p-4 flex flex-col items-center justify-center shadow transition hover:shadow-lg border border-gray-800 h-auto w-full">
        ...
        {typeof delta === "number" && (
          <span className={`mt-3 text-sm flex items-center ${delta >= 0 ? textColor : "text-red-400"}`}>
